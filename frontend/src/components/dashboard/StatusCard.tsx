...
const COLOR_MAP: Record<string, { bar: string; text: string }> = {
  ...
  "text-purple-400": { bar: "bg-accent-secondary", text: "text-accent-secondary" },
...
export default function StatusCard({
  ...
  const mapped = COLOR_MAP[color] ?? { bar: "bg-accent", text: "text-accent" };
    ...
    <div className="bg-base-surface p-5 rounded-xl border border-base-border">
      <div className="flex justify-between items-center mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-slate-400 font-medium font-body text-sm">{title}</span>
        </div>
      ...
      <div className="h-2 w-full bg-base-border rounded-full overflow-hidden">
        <div
